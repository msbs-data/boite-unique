!function() {
  ...
  function computeCalendarMonthCodeParts(calendar, year, month) {
    return calendar ? calendar.L(year, month) : (month => [ month, 0 ])(month);
  }
  ...
  function computeCalendarIsoFieldsFromParts(calendar, year, month, day) {
    return calendar ? calendar.de(year, month, day) : ((year, month, day) => ({
      year: year,
      month: month,
      day: day
    }))(year, month, day);
  ...
  function diffZonedDateTimes(invert, calendar, slots0, slots1, options) {
    ...
    if (compareBigInts(epochNano1, epochNano0)) {
      if (largestUnit < 6) {
        ...
        const timeZone = (a = slots0.timeZone, b = slots1.timeZone, a.m !== b.m && throwRangeError("Mismatching TimeZones"), 
        a);
  ...
  function diffPlainDateTimes(invert, calendar, plainDateTimeSlots0, plainDateTimeSlots1, options) {
    ...
    }))(calendar, plainDateTimeSlots0))) : durationFields = durationFieldDefaults, createDurationSlots(invert ? negateDurationFields(durationFields) : durationFields);
  }
  ...
  function diffCalendarDates(calendar, startIsoDate, endIsoDate, largestUnit) {
    ...
    if (8 === largestUnit) {
      ...
      let days = 0;
      if (sign) {
        months = calendar ? calendar._(year0, month0, year1, month1) : ((year0, month0, year1, month1) => 12 * (year1 - year0) + month1 - month0)(year0, month0, year1, month1);
  ...
  function addDurations(refineRelativeTo, doSubtract, slots, otherSlots, options) {
    ...
    return isUniformUnit(maxUnit, relativeToSlots && isZonedEpochSlots(relativeToSlots)) ? ((doSubtract, slots, otherSlots, maxUnit) => createDurationSlots(validateDurationFields(((a, b, largestUnit, doSubtract) => {
    ...
    })(slots, otherSlots, maxUnit, doSubtract))))(doSubtract, slots, otherSlots, maxUnit) : (relativeToSlots || throwRangeError("Missing relativeTo"), 
  ...
  function resolveCalendarYear(calendar, fields) {
    ...
    void 0 !== era || void 0 !== eraYear) {
      ...
      const yearByEra = exoticCalendar?.$ ? exoticCalendar.$(eraYear, normalizedEra, eraOrigin) : ((eraYear, eraOrigin) => (eraOrigin + eraYear) * (Math.sign(eraOrigin) || 1) || 0)(eraYear, eraOrigin);
  ...
  const queryTimeZoneRecord = memoize((normTimeZoneId, record) => "named" === record.kind ? new IntlTimeZone(normTimeZoneId, record.m, record.format) : new FixedTimeZone(normTimeZoneId, record.m, "fixed" === record.kind ? record.X : 0));
  class FixedTimeZone {
  ...
  const ZonedDateTime = defineTemporalClass("ZonedDateTime", class {
    ...
    round(options) {
      ...
      return createZonedDateTime(function(slots, smallestUnit, roundingInc, roundingMode) {
        ...
        if (6 === smallestUnit) {
          ...
          epochNanoseconds = roundWithMode(((epochNano, epochNano0, epochNano1) => computeEpochNanoFrac(epochNano < epochNano1 ? epochNano : epochNano1 - 1n, epochNano0, epochNano1))(epochNanoseconds, epochNano0, epochNano1), roundingMode) ? epochNano1 : epochNano0;
        } else {
          const isoDateTime = zonedEpochSlotsToIso(slots);
    ...
    getTimeZoneTransition(options) {
      const slots = getZonedDateTimeSlots(this);
      const newEpochNano = ((slots, options) => slots.timeZone.O(slots.epochNanoseconds, (options => {
