!function() {
    ...
    const queryYearData = (epochMilliToIntlFields => {
      ...
      return memoize(year => {
        ...
        do {
          ...
          monthStringsReversed.push(intlFields.ye)), epochMilli -= milliInUtcDay, (++iterations > 500 || epochMilli < -864e13) && throwRangeError();
        } while ((intlFields = epochMilliToIntlFields(epochMilli)).year >= year);
        return {
  ...
  function julianDayToUmalqura(intlUmalquraData, julianDay) {
    ...
    const {year: year, month: month, day: day} = intlUmalquraData.je(epochDaysToIsoDate(julianDay - 2440588));
    return {
      year: year,
      month: month,
      day: day
  ...
  const commonScrapedCalendarConfig = {
    ...
    R: 30,
    ve(monthCodeNumber, isLeapMonth, day) {
      if (isLeapMonth) {
