...
import { eur, nombre } from "@/lib/format";
...
function getCompteCharge(piece: Piece): { compte: string; libelle: string } {
  ...
  if (f.includes("point") || f.includes("matériaux") || f.includes("toiture")) {
    return {
      compte: "601000",
      libelle: "Achats matières premières & couverture",
    };
  }
  return {
    compte: "607000",
    libelle: piece.fournisseur ?? "Achats de marchandises",
  };
}
...
function ConformiteBanner({ piece }: { piece: Piece }) {
      ...
      <div className="flex items-start gap-3 rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-4 text-emerald-950 dark:text-emerald-200">
        ...
        <div className="space-y-1 text-xs">
          <div className="font-semibold text-sm">
            Flux certifié Factur-X (XML norme EN 16931)
          </div>
          <div className="text-emerald-850 leading-relaxed dark:text-emerald-300/90">
            Métadonnées fiscales intégrées et certifiées. Lignes d'écritures
            générées automatiquement pour Sage sans risque d'erreur de saisie.
          </div>
        </div>
      </div>
    );
        ...
        <div className="space-y-1 text-xs">
          <div className="flex flex-wrap items-center gap-2">
            <span className="font-semibold text-sm">
              Reconnaissance OCR & Vision IA appliquée
            </span>
            {piece.confiance && (
              <Badge
                variant="outline"
          ...
          <div className="text-blue-850 leading-relaxed dark:text-blue-300/90">
            Photo smartphone / ticket de caisse numérisé. Les montants HT, TVA
            et TTC ont été reconnus par OCR. Vérifiez la conformité avec
            l'aperçu du document avant validation.
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="flex items-start gap-3 rounded-xl border border-amber-500/30 bg-amber-500/10 p-4 text-amber-950 dark:text-amber-200">
      ...
      <div className="space-y-1 text-xs">
        <div className="font-semibold text-sm">
          Pièce non structurée / Document à contrôler
        </div>
        <div className="text-amber-850 leading-relaxed dark:text-amber-300/90">
          Ce document ne dispose pas de flux Factur-X certifié. Contrôlez les
          montants et le fournisseur avant de valider l'export.
        </div>
      </div>
    </div>
  );
        ...
        <div className="flex flex-col items-center gap-3">
          ...
          <div className="space-y-1">
            ...
            <p className="max-w-xs text-muted-foreground text-xs leading-relaxed">
              Sélectionnez une facture dans la liste de gauche pour consulter
              les données Factur-X extraites ou visualiser le document source.
            </p>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-background">
      ...
      <div className="shrink-0 space-y-3 border-b bg-muted/10 px-4 py-4 sm:px-6">
        <div className="flex flex-wrap items-baseline justify-between gap-4">
          ...
          <div className="shrink-0 font-extrabold text-2xl text-foreground tabular-nums">
            {piece.montant_ttc ? eur(Number(piece.montant_ttc)) : "—"}
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex flex-col gap-0.5 text-xs">
            ...
            <div className="flex flex-wrap items-center gap-2 text-muted-foreground">
              <span>
                Reçu le {piece.recue_le.replace("T", " à ").slice(0, 19)}
              </span>
              {piece.expediteur && (
                <>
                  <Separator orientation="vertical" className="h-3" />
      ...
      <Tabs defaultValue="metadonnees" className="flex min-h-0 flex-1 flex-col">
          ...
          <ScrollArea className="h-full">
            <div className="max-w-5xl space-y-6 p-4 sm:p-6 lg:p-8">
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <Card className="shadow-xs">
                  <CardHeader className="pb-3">
                    ...
                    <CardDescription>
                      Identification fiscale du tiers
                    </CardDescription>
                  </CardHeader>
                ...
                <Card className="shadow-xs">
                  <CardHeader className="pb-3">
                    ...
                    <CardDescription>
                      Numérotation et période d'exigibilité
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-3 text-sm">
                    <div>
              ...
              <Card className="shadow-xs">
                <CardHeader className="pb-3">
                  <CardTitle className="font-semibold text-sm">
                    Ventilation Financière Décomptée
                  </CardTitle>
                  <CardDescription>
                    Bases hors taxes, TVA déductible et montant net à payer
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
                    <div className="rounded-xl border bg-muted/20 p-4">
                      ...
                      <div className="mt-1.5 font-bold text-foreground text-xl tabular-nums">
                        {piece.montant_ht ? eur(Number(piece.montant_ht)) : "—"}
                      </div>
                    </div>
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <div>
                      ...
                      <CardDescription className="mt-1">
                        Schéma d'écriture à partie double équilibrée pour Sage
                        100 / Sage 1000
                      </CardDescription>
                    </div>
                    <Badge
                    ...
                    >
                      <CheckCircle2 className="size-3" /> Partie double
                      équilibrée
                    </Badge>
                  </div>
                </CardHeader>
                <CardContent className="p-0">
                  <div className="overflow-x-auto">
                    <Table>
                      ...
                      <TableBody>
                        <TableRow>
                          <TableCell className="pl-6 font-mono font-semibold text-xs">
                            AC
                          </TableCell>
                          ...
                          <TableCell className="text-right font-medium font-mono text-emerald-600 text-xs">
                            {piece.montant_ht
                              ? nombre(Number(piece.montant_ht))
                              : "0,00"}
                          </TableCell>
                        ...
                        <TableRow>
                          ...
                          <TableCell className="text-right font-medium font-mono text-emerald-600 text-xs">
                            {piece.montant_tva
                              ? nombre(Number(piece.montant_tva))
                              : "0,00"}
                          </TableCell>
                          <TableCell className="pr-6 text-right font-mono text-muted-foreground text-xs">
                            —
                        ...
                        <TableRow>
                          ...
                          <TableCell className="pr-6 text-right font-medium font-mono text-blue-600 text-xs">
                            {piece.montant_ttc
                              ? nombre(Number(piece.montant_ttc))
                              : "0,00"}
                          </TableCell>
                        </TableRow>
                      </TableBody>
              ...
              <Card className="shadow-xs">
                <CardHeader className="pb-3">
                  <CardTitle className="font-semibold text-sm">
                    Traçabilité & Empreinte Numérique
                  </CardTitle>
                  <CardDescription>
                    Contrôle d'intégrité SHA-256 et profil d'extraction Factur-X
                  </CardDescription>
                </CardHeader>
        ...
        <TabsContent
          value="apercu"
          className="m-0 min-h-0 flex-1 overflow-hidden p-4 sm:p-6"
        >
                    ...
                    <div className="flex flex-col items-center gap-3 p-6 text-center text-muted-foreground">
                      ...
                      <div className="space-y-1">
                        ...
                        <p className="max-w-xs text-xs">
                          Le document source ne peut être affiché directement.
                      ...
                      >
                        ...
                        >
                          <ExternalLink className="size-3.5" /> Ouvrir en plein
                          écran
                        </a>
                      </Button>
                    </div>
