...
export function PiecesTableClient() {
  ...
  const { lastUpdated, fetchStats, recevoirEmails, isReceiving } =
    useCabinetStore();
  ...
  const [selectedDossier, setSelectedDossier] = React.useState<string>(
    dossierParam || "all",
  );
  ...
  const handleValider = async (id: number) => {
    try {
      ...
      setPieces((prev) =>
        prev.map((p) =>
          p.id === id ? { ...p, etat: "lue", validee_par: "Utilisateur" } : p,
        ),
      );
        ...
        <div className="flex flex-col gap-1">
          <div className="flex items-center gap-2">
            ...
            <Badge variant="outline" className="font-mono text-xs">
              {filteredPieces.length} ligne
              {filteredPieces.length > 1 ? "s" : ""}
            </Badge>
          </div>
          <p className="text-xs text-muted-foreground">
            Vue tabulaire haute densité : consultation des pièces, vérification
            et métadonnées fiscales
          </p>
        </div>
                    ...
                    >
                      ...
                      <TableCell>
                        {piece.type === "structure" ? (
                          <Badge
                            variant="secondary"
                            className="gap-1 bg-emerald-500/10 text-emerald-600 text-[10px]"
                          >
                      ...
                      >
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          >
                            <a
                              href={api.getPieceFileUrl(piece.id)}
                              target="_blank"
                              rel="noreferrer"
                            >
            ...
            <>
              ...
              <DialogHeader className="p-4 sm:p-5 border-b bg-muted/20 shrink-0 pr-12 text-left">
                ...
                <DialogDescription className="text-xs text-muted-foreground flex flex-wrap items-center gap-x-3 gap-y-1 mt-0.5">
                  <span>
                    Reçu le{" "}
                    {activePiece.recue_le?.replace("T", " à ").slice(0, 19)}
                  </span>
                  <span>&bull;</span>
                ...
                <div className="flex flex-col gap-4">
                    ...
                    <div className="flex items-start gap-3 rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-3.5 text-xs text-emerald-950 dark:text-emerald-200">
                      ...
                      <div>
                        <div className="font-semibold text-xs">
                          Factur-X certifié (Profil{" "}
                          {activePiece.profil || "EN 16931"})
                        </div>
                        <div className="text-emerald-850 dark:text-emerald-300/90 text-[11px] mt-0.5 leading-relaxed">
                          Format mixte PDF + XML normalisé. Les données fiscales
                          et montants sont certifiés conformes sans saisie
                          manuelle.
                        </div>
                      </div>
                    </div>
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <span className="font-semibold text-xs">
                            Photo Smartphone & Reconnaissance Vision OCR
                          </span>
                          {activePiece.confiance && (
                            ...
                            >
                              Confiance OCR{" "}
                              {Math.round(activePiece.confiance * 100)}%
                            </Badge>
                          )}
                        </div>
                        <div className="text-blue-850 dark:text-blue-300/90 text-[11px] mt-0.5 leading-relaxed">
                          Ticket de caisse / document photographié. Les montants
                          ont été extraits par OCR.
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div className="rounded-xl border p-3 bg-card/60 space-y-1">
                      ...
                      <div className="font-semibold text-sm truncate">
                        {activePiece.fournisseur || "Non renseigné"}
                      </div>
                      <div className="text-xs text-muted-foreground font-mono">
                        SIREN : {activePiece.siren || "—"}{" "}
                        {activePiece.tva_intracom &&
                          `• TVA : ${activePiece.tva_intracom}`}
                      </div>
                    </div>
                    ...
                    <div className="rounded-xl border p-3 bg-card/60 space-y-1">
                      ...
                      <div className="font-semibold text-sm font-mono truncate">
                        {activePiece.numero || "—"}
                      </div>
                      <div className="text-xs text-muted-foreground">
                        Date : {activePiece.date_facture || "—"}
                      </div>
                    </div>
                  </div>
                      ...
                      <div className="border rounded-lg p-2.5 bg-muted/20">
                        ...
                        <div className="font-semibold text-sm tabular-nums mt-0.5">
                          {activePiece.montant_ht
                            ? eur(Number(activePiece.montant_ht))
                            : "—"}
                        </div>
                      </div>
                      <div className="border rounded-lg p-2.5 bg-muted/20">
                        ...
                        <div className="font-semibold text-sm tabular-nums text-primary mt-0.5">
                          {activePiece.montant_tva
                            ? eur(Number(activePiece.montant_tva))
                            : "—"}
                        </div>
                      </div>
                      <div className="border rounded-lg p-2.5 bg-emerald-500/10 border-emerald-500/30">
                        ...
                        <div className="font-bold text-base tabular-nums text-emerald-600 dark:text-emerald-400 mt-0.5">
                          {activePiece.montant_ttc
                            ? eur(Number(activePiece.montant_ttc))
                            : "—"}
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="rounded-xl border bg-card overflow-hidden">
                    <div className="bg-muted/40 px-3.5 py-2 border-b flex items-center justify-between">
                      <span className="font-semibold text-xs text-foreground">
                        Écriture comptable Sage 100 / 1000
                      </span>
                        ...
                        <TableBody>
                          <TableRow>
                            ...
                            <TableCell className="text-right font-mono text-emerald-600">
                              {activePiece.montant_ht
                                ? nombre(Number(activePiece.montant_ht))
                                : "0,00"}
                            </TableCell>
                          ...
                          <TableRow>
                            ...
                            <TableCell className="text-right font-mono text-emerald-600">
                              {activePiece.montant_tva
                                ? nombre(Number(activePiece.montant_tva))
                                : "0,00"}
                            </TableCell>
                            <TableCell className="text-right font-mono text-muted-foreground">
                              —
                          ...
                          <TableRow>
                            ...
                            <TableCell className="text-right font-mono font-semibold text-blue-600">
                              {activePiece.montant_ttc
                                ? nombre(Number(activePiece.montant_ttc))
                                : "0,00"}
                            </TableCell>
                          </TableRow>
                        </TableBody>
                  ...
                  <div className="rounded-xl border p-3 bg-muted/10 text-xs space-y-1">
                    ...
                    <div
                      ...
                      title={activePiece.empreinte}
                    >
                      Empreinte SHA-256 :{" "}
                      <span className="font-mono text-[10px] text-foreground">
                        {activePiece.empreinte}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-col rounded-xl border bg-muted/10 overflow-hidden min-h-[440px]">
                  <div className="flex items-center justify-between border-b bg-card px-3.5 py-2 text-xs">
                    <div className="flex items-center gap-1.5 truncate">
                      <span className="font-mono truncate font-medium max-w-[180px]">
                        {activePiece.nom_fichier}
                      </span>
                      {activePiece.type === "image" ||
                  ...
                  <div className="flex-1 flex items-center justify-center p-3 bg-muted/20 overflow-auto">
                    {activePiece.type === "image" ||
                    /\.(jpg|jpeg|png|webp)$/i.test(activePiece.nom_fichier) ? (
                      previewImageError ? (
                        <div className="flex flex-col items-center gap-2 p-6 text-center text-muted-foreground">
                          ...
                          >
                            <a
                              ...
                              rel="noreferrer"
                            >
                              <ExternalLink className="size-3" /> Ouvrir en
                              plein écran
                            </a>
                          </Button>
                        </div>
