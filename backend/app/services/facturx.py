...
XML. Quand ce XML est présent, on le lit — on ne regarde jamais l'image.
"""
...
def lire_cii(xml_octets: bytes) -> FactureStructuree:
    """Lit un CrossIndustryInvoice et en tire les champs d'en-tête en désactivant les entités externes (XXE)."""
    ...
    racine = etree.fromstring(xml_octets, parser=parser)
    ...
    return FactureStructuree(
        ...
        profil=_texte(racine, "rsm:ExchangedDocumentContext/ram:GuidelineSpecifiedDocumentContextParameter/ram:ID"),
    )
